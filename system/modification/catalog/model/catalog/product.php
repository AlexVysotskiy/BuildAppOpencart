...
class ModelCatalogProduct extends Model
{
    ...
    {
        $query = $this->db->query("
            SELECT * FROM " . DB_PREFIX . "product_to_category ptc WHERE product_id = '" . (int) $product_id . "'
        ");
    ...
    public function getTotalProductsFranchise($data = array())
    {
        $sql = "SELECT COUNT(DISTINCT p.product_id) AS total";
        ...
        $sql .= "  
            ...
            ON (p.user_group_id = u.user_group_id)
            LEFT JOIN " . DB_PREFIX . "product_to_store p2s 
            ON (p.product_id = p2s.product_id) 
            WHERE pd.language_id = '" . (int) $this->config->get('config_language_id') . "' 
            AND p.status = '1' AND p.date_available <= NOW() 
            AND p2s.store_id = '" . (int) $this->config->get('config_store_id') . "'
            AND u.zone_id = '" . $this->session->data['zone_id'] . "'
        ";
    ...
    public function getProductUserGroupId($product_id)
    {
        $query = $this->db->query("
